...
@section('title') Create New CMS User @endsection
@section('page_pre_title')  @endsection
@section('page_title') Create New CMS User @endsection

@section('content')
<div class="col-md-8">
	<div class="card">
		<div class="card-body">
			{!! Form::open(['url' => route('cms.users.add')]) !!}
						...
						@if(empty($roles) == false && $roles->count() > 0)
							@foreach($roles as $role)
								<label class="form-check col-4">
		                            <input type="checkbox" name="roles[]" @if(is_array(old('roles')) && in_array($role->id, old('roles'))) checked @endif class="form-check-input" value="{{ $role->id }}" />
